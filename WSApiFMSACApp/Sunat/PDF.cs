    ...
    public class PDF
    {
		...
		public bool EnviarPDFCorreo(Transaccion oTransaccionVenta, Empresa p_oEmpresa, string strNombreArchivo)
		{
			try
			{
				...
				else if (oTransaccionVenta.TipoDocumento_Id == (int)Enumerador.FACTURA_ELECTRONICA.TIPO_DOCUMENTO_GUIA_REMISION)
				{
					RUTA_PDF = p_oEmpresa.sEmpRuta + Enumerador.RUTA_SERVIDOR_GUIA_REMISION_ELECTRONICAS_PDF;
					//sAsunto = "Te ha llegado una Nota de Débito Electrónica";
					//sMensajeTexto = "Estimado Cliente, Su Nota de Débito ha sido emitida a SUNAT";
				}
						...
						if (oTransaccionVenta.TipoDocumento_Id == (int)Enumerador.FACTURA_ELECTRONICA.TIPO_DOCUMENTO_NOTA_CREDITO)
				{
					...
					if (File.Exists(RUTA_XML))
					{
						...
						if (oTransaccionVenta.TipoDocumento_Id == (int)Enumerador.FACTURA_ELECTRONICA.TIPO_DOCUMENTO_NOTA_DEBITO)
				{
					string RUTA_XML = string.Empty;

					RUTA_XML = oEmpresa.sEmpRuta + Enumerador.RUTA_SERVIDOR_DOCUMENTO_ELECTRONICO_NOTA_DEBITO +
							   strNombreArchivo + @"\" +
							   strNombreArchivo + ".xml";
					...
					if (File.Exists(RUTA_XML))
					{
						...
						if (oTransaccionVenta.TipoDocumento_Id == (int)Enumerador.FACTURA_ELECTRONICA.TIPO_DOCUMENTO_GUIA_REMISION)
				{
					string RUTA_XML = string.Empty;
		...
		public bool ReportarGuiaRemision(DataTable dtSource, Transaccion oTransaccion, string strNombreArchivo, Empresa oEmpresa)
		{
			try
			{
				oTransaccionVenta = null;
				...
				var reporte = new rptFormatoGuiaRemision();
				return MostrarReporteGuiaRemision(dtSource, reporte, strNombreArchivo, oEmpresa);
			}
			catch
			{
				return false;
		...
		private void sReporte_RefreshReport(object sender, EventArgs e)
		{
			try
			{
				...
				var nTotalIGV = oTransaccionVenta.nTraMontoIGV;
				...
				decimal prmTotalGravado = 0;
				decimal prmTotalNoGravado = 0;
				decimal prmTotalExonerado = 0;

				prmTotalGravado = oTransaccionVenta.LstTransaccionDetalle.Where(x => x.nTDeEstadoIGV == (int)Enumerador.FACTURA_ELECTRONICA.TIPO_IGV_GRABADO).Sum(x => x.nTDeBase);
				sReporte.SetParameterValue("prmTotalGravado", prmTotalGravado);

				prmTotalNoGravado = oTransaccionVenta.LstTransaccionDetalle.Where(x => x.nTDeEstadoIGV == (int)Enumerador.FACTURA_ELECTRONICA.TIPO_IGV_NO_GRABADO).Sum(x => x.nTDeBase);
				sReporte.SetParameterValue("prmTotalNoGravado", prmTotalNoGravado);

				prmTotalExonerado = oTransaccionVenta.LstTransaccionDetalle.Where(x => x.nTDeEstadoIGV == (int)Enumerador.FACTURA_ELECTRONICA.TIPO_IGV_EXONERADO).Sum(x => x.nTDeBase);
				sReporte.SetParameterValue("prmTotalExonerado", prmTotalExonerado);

				//sReporte.SetParameterValue("prmTotalGravado", oTransaccionVenta.nTraSubTotalSinIGV);

				sReporte.SetParameterValue("prmImpuestoICBPER", oTransaccionVenta.nTraMontoICBPER);
		...
		private bool MostrarReporteGuiaRemision(DataTable dtSource, rptFormatoGuiaRemision sReporte, string strNombreArchivo, Empresa p_oEmpresa)
		{
			try
			{
				...
				Directory.CreateDirectory(oEmpresa.sEmpRuta + Enumerador.RUTA_SERVIDOR_GUIA_REMISION_ELECTRONICAS_PDF +
										  strNombreArchivo);


				string RUTA_PDF = oEmpresa.sEmpRuta + Enumerador.RUTA_SERVIDOR_GUIA_REMISION_ELECTRONICAS_PDF +
								  strNombreArchivo + @"\" +
								  strNombreArchivo + ".pdf";
				...
				sReporte.SetDataSource(dtSource);
				sReporte.RefreshReport += sReporteGuias_RefreshReport;
		...
		private void sReporteNotas_RefreshReport(object sender, EventArgs e)
		{
			try
			{
				...
				{
					sReporte.SetParameterValue("prmTipoDocumento", "Boleta");
					sReporte.SetParameterValue("prmNumeroAfecto", "" + oTransaccionVenta.sSerieNumeroRelacionado);
				}
				else
				{
		...
		private void sReporteGuias_RefreshReport(object sender, EventArgs e)
		{
			try
			{
				var sReporte = ((ReportDocument)sender);
				...
				sReporte.SetParameterValue("prmDireccionEmpresa", oEmpresa.sEmpDireccion);
				sReporte.SetParameterValue("prmRucEmpresa", oEmpresa.sEmpRuc);
				sReporte.SetParameterValue("prmNroCorrelativo", oTransaccionVenta.sNumeroGenerado);

				sReporte.SetParameterValue("prmFechaEmisión", oTransaccionVenta.dTraFecha);
				sReporte.SetParameterValue("prmFechaTraslado", oTransaccionVenta.dTraFechaTraslado);
				sReporte.SetParameterValue("prmMotivoTraslado", oTransaccionVenta.sTraMotivoTraslado);

				if (oTransaccionVenta.sTraCodigoModalidadTraslado.toInt() == 1)
				{
					sReporte.SetParameterValue("prmModalidadTransporte", "Transporte Público");
				}
				else
				{
					sReporte.SetParameterValue("prmModalidadTransporte", "Transporte Privado");
				}


				string TextoUnidadPesoGuia = string.Empty;

				TextoUnidadPesoGuia =
					"Peso Bruto Total de la Guía (" + oTransaccionVenta.sTraCodigoUnidadMedidaTraslado + ") : ";

				sReporte.SetParameterValue("prmTextoUnidadPesoGuia", TextoUnidadPesoGuia);
				sReporte.SetParameterValue("prmPesoTraslado", oTransaccionVenta.nTraPesoBrutoGuiaTraslado);

				//Destinatario
				sReporte.SetParameterValue("prmDestinatario", oTransaccionVenta.sCliente);
				sReporte.SetParameterValue("prmRUCDestinatario", oTransaccionVenta.sCliNroIdentidad);

				//Datos del Transportista
				sReporte.SetParameterValue("prmNombreTransportista", oTransaccionVenta.sTraNombreTransportistaTraslado);
				sReporte.SetParameterValue("prmRUCTransportista", oTransaccionVenta.sTraRUCTransportistaTraslado);

				//Datos del Punto de Partida y Punto de Llegada
				sReporte.SetParameterValue("prmDireccionPuntoPartida", oTransaccionVenta.sTraDireccionPuntoPartidaTraslado);
				sReporte.SetParameterValue("prmDireccionPuntoLlegada", oTransaccionVenta.sTraDireccionPuntoLlegadaTraslado);
				sReporte.SetParameterValue("prmObservaciones", oTransaccionVenta.sTraObservaciones);

				sReporte.SetParameterValue("prmPlacaVehiculo", oTransaccionVenta.sTraPlacaVehiculoTraslado);
				sReporte.SetParameterValue("prmDNIConductor", oTransaccionVenta.sTraDNIConductorTraslado);
